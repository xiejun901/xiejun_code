...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet ={
    ...
    val maxElem = try{
      ...
      if(leftElem.retweets > elem.retweets) leftElem
      else elem
    } catch {
    ...
    try {
      ...
      if(rightElem.retweets > maxElem.retweets) rightElem
      else maxElem
    } catch {
      case e: NoSuchElementException => maxElem
      case e2: Throwable => throw e2
    }
  }

  def descendingByRetweet = {
    val mostElem = mostRetweeted
    new Cons(mostElem, remove(mostElem).descendingByRetweet)
  }
  /**
   * The following methods are already implemented
   */
