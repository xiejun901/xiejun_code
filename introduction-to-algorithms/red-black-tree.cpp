...
public:
	...
	void rbtDelete(rbtPtr z)
	{
		...
		//从整棵树的情况来看，其实是删掉了结点y本来所占位置的结点，因此如果结点y本来所占位置的结点的颜色是红色的
		//那么所有结点的的性质5，从该结点到其子孙结点的所有路径上经过的黑色结点数目相同，因为删掉的是红色结点，所以这个数目是没变的
		//既然y本来所占的位置是红色的，那么y就不可能是根结点
...
private:
	...
	void rbtDeleteFix(rbtPtr x)
	{
		while (x != root&&x->color == rbBlack)
		{
			...
			if (x == x->parent->left)
			{
				rbtPtr w = x->parent->right;//w:x叔父结点
				if (w->color == rbRed)
				{
